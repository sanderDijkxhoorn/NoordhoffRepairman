...
function npShowAnswers() {
  ...
  for (i = 0; i < interactionContainers.length; i++) {
    ...
    if (
      window
        .getComputedStyle(interactionContainers[i])
  ...
  if (interaction.type === "dragAndDropInteraction") {
    const dropAreas =
      interactionContainers[i].getElementsByClassName("drop-area");
    for (var j = 0; j < dropAreas.length; j++) {
      const dropItem = dropAreas[j].getElementsByClassName("drop-item")[0];

      var answers = dropAreas[j].getElementsByClassName("ui-droppable-correct");
      console.log(answers.length);
      for (var k = 0; k < answers.length; k++) {
        const answerTitle = answers[k].children[0].getAttribute("title");
        console.log("Answer: " + answerTitle);

        // answers[k].setAttribute("answerid", "henk");

        const dragItem = document.getElementById(
          answers[k].getAttribute("answerid")
        );

        console.log(dragItem);
        const dragItemTitle = dragItem
          .getElementsByClassName("text-item")[0]
          .getAttribute("title");

        if (answerTitle === dragItemTitle) {
          var isAlreadyCorrect = false;
          for (let l = 0; l < dropItem.children.length; l++) {
            console.log(dropItem.children[l]);
            console.log(dragItem.getElementsByClassName("drag-item")[0]);
            console.log(
              dropItem.children[l].getAttribute("id") ===
                dragItem.getAttribute("id")
            );
            if (
              dropItem.children[l].getAttribute("id") ===
              dragItem.getAttribute("id")
    ...
    const dropdownboxes =
      interactionContainers[i].getElementsByClassName("dropdown-box");
    for (var j = 0; j < dropdownboxes.length; j++) {
      var choices =
        dropdownboxes[j].getElementsByClassName("drop-box")[0].children;
    ...
    const texteditor =
      interactionContainers[i].querySelector("iframe").contentDocument.body;

    const answerElement = interactionContainers[i].querySelector(
      "#plselfScoreFeedback > div.plselfScoreFeedbackAnswer.clearfix"
    );
    ...
    var answers =
      interactionContainers[i].getElementsByClassName("toggleAnswer");
    ...
    console.log(interactionContainers[i]);
    var answers =
      interactionContainers[i].getElementsByClassName("correctAnswer");
    for (var j = 0; j < answers.length; j++) {
      console.log(answers[j]);
      answers[j].parentElement.getElementsByClassName("answerInput")[0].value =
        answers[j].innerHTML;
    }
  } else {
